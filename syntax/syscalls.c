...
void syscall_read() {
    char c;
    int val = (read(STDIN_FILENO, &c, 1) == 1) ? (unsigned char) c : EOF;
    printf("%c", (char)val);
}
...
// 文件描述符通常是一个小的非负整数，内核用以标识一个特定进程正在访问的文件。当内核打开一个现有
// 文件或创建一个新文件时，它都返回一个文件描述符。在读、写文件时，可以使用这个文件描述符。

// 按惯例，每当运行一个新程序时，所有的shell都为其打开3个文件描述符，即标准输入、标准输出、
// 标准错误。如果不做特殊处理，则这3个描述符都链接到终端。（例：STDIN_FILENO）
